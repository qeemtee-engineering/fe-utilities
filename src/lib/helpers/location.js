...
export const getFormattedLocation = location => {
  ...
  location.addressLines.forEach(line => (line ? (addressLinesCombined += `${line}, `) : ''));
  return `${addressLinesCombined} ${location.city}, ${location.country} `;
};

export const getLocationUrl = location => `https://www.google.com/maps?z=12&t=m&q=loc:${location.lat}+${location.long}`;
...
export const getPickUpLocation = (activity, currentBooking, hostdata) => {
  ...
  if (get(activity, 'pickUp.isAvailable')) {
    ...
    const pickUp = cloneDeep(activity.pickUp);
    ...
    if (get(activity, 'pickUp.radius')) {
      ...
      if (
        ...
        pickUp.location.url = getLocationUrl(get(hostdata, 'office.address'));
      } else if (
        (get(activity, 'pickUp.location'), get(activity, 'pickUp.radius'))
      ) {
        // Set location url to default merchant base pickup location
        pickUp.location.url = getLocationUrl(get(activity, 'pickUp.location'));
      }
